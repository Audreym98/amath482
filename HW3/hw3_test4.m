...
%%
%Play Videos
frames1 =size(vidFrames1_4);
frames2 =size(vidFrames2_4);
frames3 =size(vidFrames3_4);
...
filter(300-1.5*width:1:300+3*width, 350-1.5*width:1:350+2.5*width) = 1; % black out around paint can
...
for j=1:frames1(4)
    ...
    X = double(rgb2gray(X));
    Xf = X.*filter;
    thresh = Xf > 246; % pick out bright points
...
filter(250-4*width:1:250+3*width, 290-2.5*width:1:290+3*width) = 1;
...
filter = zeros(480,640);
filter(250-3*width:1:250+1*width, 360-2*width:1:360+3*width) = 1;
...
title("Test 4: Energy of Each Mode");
...
xlabel("Modes"); ylabel("Energy");
subplot(3,1,2)
plot(1:min, Y(1,:), 1:min, Y(2,:),1:min,Y(3,:),'r','Linewidth', 2)
ylabel("Displacement"); xlabel("Time"); 
title("Test 4: Projection Onto Principal Components");
legend("PC1", "PC2", "PC3")
subplot(3,1,3)
plot(1:min, X(2,:),1:min, X(1,:), 'Linewidth', 2)
ylabel("Displacement"); xlabel("Time"); 
title("Test 4: Original displacement (cam 1)");
legend("Z", "X-Y")